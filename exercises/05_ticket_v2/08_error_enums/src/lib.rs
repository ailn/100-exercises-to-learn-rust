use crate::TicketNewError::{DescriptionInvalid, TitleInvalid};
...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description, status.clone()) {
        ...
        Err(error) => match error {
            DescriptionInvalid(_) => Ticket::new(title, "Description not provided".to_string(), status).unwrap(),
            TitleInvalid(msg) => panic!("{msg}")
        }
    }
}
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TitleInvalid("Title cannot be empty".to_string()));
        }
        if title.len() > 50 {
            return Err(TitleInvalid("Title cannot be longer than 50 bytes".to_string()));
        }
        if description.is_empty() {
            return Err(DescriptionInvalid("Description cannot be empty".to_string()));
        }
        if description.len() > 500 {
            return Err(DescriptionInvalid("Description cannot be longer than 500 bytes".to_string()));
        }
