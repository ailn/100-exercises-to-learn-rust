...
{
    let boxed_reply = Arc::new(reply);
    let handle1 = tokio::spawn(internal_fixed_reply(first, boxed_reply.clone()));
    let handle2 = tokio::spawn(internal_fixed_reply(second, boxed_reply));
    let (outcome1, outcome2) = tokio::join!(handle1, handle2);
}

async fn internal_fixed_reply<T>(listener: TcpListener, reply: Arc<T>) -> Result<(), anyhow::Error>
where
// `T` cannot be cloned. How do you share it between the two server tasks?
    T: Display + Send + Sync + 'static,
{
